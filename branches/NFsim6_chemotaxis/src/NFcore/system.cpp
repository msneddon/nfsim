...
double System::getAverageGroupValue(char * groupName, int valIndex)
{
	...
	{
		const char * name = (*groupIter)->getName();
...
{
//	if(moleculeTypeIndex>allMoleculeTypes.size())
//	{
//		
//	} else if( observableIndex>allMoleculeTypes.at(moleculeTypeIndex)->)
//	{
//		
//	}
//	cout<<"here"<<endl;
...
void System::prepareForSimulation()
{
	...
	for(rxnIter = allReactions.begin(); rxnIter != allReactions.end(); rxnIter++ )
  	...
  	cout<<"DEBUG\tSystem '"<<this->name<<"' is ready for simulation."<<endl;
  	
  	
  	
	cout<<"DEBUG\tAll Molecules:"<<endl;
	int idxCounter = 0;
	for(molTypeIter = allMoleculeTypes.begin(); molTypeIter != allMoleculeTypes.end(); molTypeIter++ )
	{
		cout<<"DEBUG\t\t"<<idxCounter++<<"\t"<<(*molTypeIter)->getName()<<" (#=";
		cout<<(*molTypeIter)->getMoleculeCount()<<")"<<endl;
	}
	cout<<"DEBUG\tAll System Rxns:"<<endl;
	idxCounter = 0;
	for(rxnIter = allReactions.begin(); rxnIter != allReactions.end(); rxnIter++ )
	{
		cout<<"DEBUG\t\t"<<idxCounter++<<"\t"<<(*rxnIter)->getName()<<" (rate = ";
		cout<<(*rxnIter)->getRate()<<")"<<endl;
	}
	cout<<endl;
...
double System::sim(double duration, long int sampleTimes)
{
	...
	while(current_time<end_time)
	{
		...
		if(a_tot>0) delta_t = -log(NFutil::RANDOM_OPEN()) / a_tot;
