...
{
	//To create any reaction, we have to create template molecules
	//that define the minimal constraints for a molecule to be a reactant in this reaction
	int n_reactants = 1;
	TemplateMolecule ** reactantTemplates = new TemplateMolecule *[n_reactants];
	
	reactantTemplates[0] = new TemplateMolecule(molX);
	reactantTemplates[0]->addStateValue("p",1); //Set constraint that we are phosphorylated
	
	//Creating this reaction tells us the name, the number of reactants, the template molecules,
	//the rate, and in this case, that state "p" should go to value 0 in this rxn.  These
	//reactions "types" are defined in the file "/src/NFreactions/basic/NFreactions.hh"
	return new ReactionChangeState("X_dephos",n_reactants,reactantTemplates, rate, "p",0);
}
...
ReactionClass * NFtest_simple_system::createReactionYphosX(MoleculeType *molX, MoleculeType *molY, double rate)
{
	...
	reactantTemplates[0] = new TemplateMolecule(molX);
	reactantTemplates[0]->addStateValue("p",0);  //Set constraint that we are not phosphorylated
	TemplateMolecule * yTemplate = new TemplateMolecule(molY);
	TemplateMolecule::bind(reactantTemplates[0],"y",yTemplate,"x");
	
	return new ReactionUnbindAndChangeState("Y_phos_X",n_reactants,reactantTemplates, rate,"y","p",1);
...
ReactionClass * NFtest_simple_system::createReactionXYbind(MoleculeType *molX,MoleculeType *molY, double rate)
{
	...
	reactantTemplates[0] = new TemplateMolecule(molX);
	reactantTemplates[0]->addEmptyBindingSite("y");
	reactantTemplates[0]->addStateValue("p",0);  //Only bind if we are unphosphorylated
	reactantTemplates[1] = new TemplateMolecule(molY);
	reactantTemplates[1]->addEmptyBindingSite("x");
	
	return new ReactionSimpleBinding("XY_binding", n_reactants, reactantTemplates, rate,"y","x");
}
...
ReactionClass * NFtest_simple_system::createReactionXYunbind(MoleculeType *molX, MoleculeType *molY, double rate)
{
	...
	return new ReactionUnbinding("XY_unbinding", n_reactants, reactantTemplates, rate,"y");
}
