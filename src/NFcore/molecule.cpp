...
{
	list <StateChangeListener *>::iterator listenerIter; 
...
double Molecule::getDORvalueFromGroup(const char * groupName, int valueIndex)
{
	list <StateChangeListener *>::iterator listenerIter; 
	for(listenerIter = listeners.begin(); listenerIter != listeners.end(); listenerIter++ )
	{
	//	cout<<"here: "<<groupName<<endl;
			if(strcmp(groupName,(*listenerIter)->getGroupName())==0)
...
void Molecule::bind(Molecule *m1, int bSiteIndex1, Molecule *m2, int bSiteIndex2)
{
	if(m1->bonds[bSiteIndex1]!=0 || m2->bonds[bSiteIndex2]!=0) { 
		cerr<<"Trying to bond "<< m1->getMoleculeTypeName() << "(";
		cerr<<m1->getMoleculeType()->getBindingSiteName(bSiteIndex1)<<") & ";
		cerr<< m2->getMoleculeTypeName()<<"(";
		cerr<<m2->getMoleculeType()->getBindingSiteName(bSiteIndex2)<<")\n";
