...
MoleculeType * createReceptorDimer(System * s, NGparam &p)
{
	...
	int numOfBsites; const char ** bSiteNames;
	if(p.get_useNeighborRxns() && p.get_useTether()) {
		numOfBsites = 8; bSiteNames = new const char * [numOfBsites];
		bSiteNames[0] = "t0";
		bSiteNames[1] = "t1";
		bSiteNames[2] = "t2";
		bSiteNames[3] = "t3";
		...
		numOfBsites = 2; bSiteNames = new const char * [numOfBsites];
		bSiteNames[0] = p.get_nameReceptorTetherSite();
		bSiteNames[1] = p.get_nameReceptorActiveSite();
	} else if(!p.get_useNeighborRxns() && !p.get_useTether()) {
		numOfBsites = 1; bSiteNames = new const char * [numOfBsites];
...
MoleculeType * createCheB(System *s, NGparam &p)
{
	...
	int numOfStates; const char ** stateNames; int * stateValues;
	if(p.get_useCheBFeedback())
	{
		numOfStates = 1;
		stateNames = new const char * [numOfStates];
