...
/* Structure used to for species and rate constants.  For species, val is the
concentration in a consistent set of units (up to user).*/
typedef struct ELT{
    char           *name;
    double          val;
    int             index;
...
/* Structure used to hold an array of Elt structures.  Used to speed up
   finding of an elt by index. */
typedef struct {
    int n_elt;
    int offset;
    Elt **elt;
...
/* Structure that contains a listing of elt indices that defines a group of
   elts */
...
/* Structure used to contain reactions.  Indices of r_index and p_index refer
to an Elt list (or Elt_array) containing species, while rate_index refers to
an Elt list or Elt_array containing rates. */
typedef struct RXN{
  int             index;
  int             n_reactants;
  int             n_products;
...
/* Structure used to contain an array of reactions that can be quickly
   accessed by index. */
typedef struct {
    int n_rxn;
    int offset;
...
extern char **parse_line (char *buf, int *n_tok, char *comment_chars,
			  char *sep_chars);
...
extern void add_Elt( Elt **list, Elt *new);
...
extern Elt_array *read_Elt_array(  FILE *datfile, int *line_number,
				   char *name, int *n_read, Elt_array *params);

/* Group handling routines */
extern Group *add_Group( Group *glist, char *name, int index, int n_elt, int *elt_index,
			 double *elt_factor);
extern void free_Group( Group *group);
extern Group *read_Groups(  Group *glist, FILE *datfile, Elt_array *earray, int *line_number,
...
extern void add_Rxn( Rxn **list, Rxn *new);
...
extern Rxn_array *read_Rxn_array( FILE *datfile, int *line_number, int *n_read,
				  Elt_array *species, Elt_array *rates);
extern void print_Rxn_array( FILE *outfile, Rxn_array *reactions,
			     Elt_array *species, Elt_array *rates);
...
extern int init_network( Rxn_array *reactions, Elt_array *rates,
			 Elt_array *species, Group *spec_groups,
			 char *name);
...
extern int propagate_cvode_network( double *t, double delta_t,
    long int *n_steps, double *rtol, double *atol, int SOLVER_TYPE);
extern int propagate_euler_network( double *t, double delta_t, long int *n_steps, double h);
extern int propagate_rkcs_network ( double *t, double delta_t, long int *n_steps, double tol);
extern FILE *init_print_concentrations_network( char *filename);
extern int print_concentrations_network( FILE *out, double t);
extern int finish_print_concentrations_network( FILE *out);
extern FILE *init_print_group_concentrations_network( char *filename);
...
extern void connectivity_Rxn_array( Rxn_array *reactions, Elt_array *species,
...
extern int gillespie_direct_network( double *t, double delta_t,
