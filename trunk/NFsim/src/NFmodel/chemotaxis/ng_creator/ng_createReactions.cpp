		...
		NGparam &p)
{
	...
	ReactionClass * r = new ReactionSimpleBinding("FREE_B_bind_TETHER", 
		n_reactants, reactantTemplates, p.get_rateFREE_CHEB_bind_TETHER(), "tether", "te");
	...
	TemplateMolecule::bind(reactantTemplates[0], "te", receptorTemplate, "tether");
	r = new ReactionUnbinding("B_unbind_TETHER",
		n_reactants,reactantTemplates,p.get_rateCHEB_unbind_TETHER(),"te");
		...
		NGparam &p)
{
	...
	TemplateMolecule ** reactantTemplates = new TemplateMolecule *[n_reactants];
	reactantTemplates[0] = new TemplateMolecule(receptorDimer);
	reactantTemplates[0]->addEmptyBindingSite(p.get_nameReceptorTetherSite());
	reactantTemplates[1] = new TemplateMolecule(cheR);
	reactantTemplates[1]->addEmptyBindingSite(p.get_nameCheRtetherSite());
	...
	ReactionClass * r = new ReactionSimpleBinding("FREE_R_bind_TETHER", 
		n_reactants, reactantTemplates, p.get_rateFREE_CHER_bind_TETHER(), "tether", "te");
	r->setTraversalLimit(2);
	s->addReaction(r);
	...
	r = new ReactionUnbinding("R_unbind_TETHER",
		n_reactants,reactantTemplates,p.get_rateCHER_unbind_TETHER(),"te");
	r->setTraversalLimit(3);
	s->addReaction(r);
}
		...
		NGparam &p)
{
	...
	reactantTemplates[0]->addStateValue(p.get_nameCheAphosState(),UNPHOS);  //Set constraint that p is unphos	
	...
	ReactionClass * autoPhosA = new NG_AutoPhosA("AUTO_PHOS_A",n_reactants,reactantTemplates,DORreactantIndex,DORgroupName,DORgroupValueIndex,
		p.get_rateAUTO_phos_CHEA(),p.get_nameCheAphosState(),PHOS);
		...
		NGparam &p)
{
	...
	reactantTemplates[0] = new TemplateMolecule(cheA);
	reactantTemplates[0]->addStateValue(p.get_nameCheAphosState(),PHOS);  //Set constraint that cheA is at p=1
	
	reactantTemplates[1] = new TemplateMolecule(cheY);
	reactantTemplates[1]->addStateValue(p.get_nameCheYphosState(),UNPHOS);  //Set constraint that cheY is at p=0
	
	ReactionClass * r = new ReactionChangeStateOfTwoMolecules("A_PHOS_Y", n_reactants, reactantTemplates,
		p.get_rateCHEA_phos_CHEY(),p.get_nameCheAphosState(),UNPHOS, p.get_nameCheYphosState(), PHOS);
		...
		NGparam &p)
{
	...
	reactantTemplates[0]->addStateValue(p.get_nameCheYphosState(),PHOS);  //Set constraint that we are at p=1
	
	ReactionClass * r = new ReactionChangeState("Y_AUTO_DEPHOS", n_reactants, reactantTemplates, 
			p.get_rateAUTO_dephos_CHEY(),p.get_nameCheYphosState(),UNPHOS);
	r->setTraversalLimit(1);
	s->addReaction(r);
}
		...
		NGparam &p)
{
	...
	reactantTemplates[1] = new TemplateMolecule(cheB);
	reactantTemplates[1]->addStateValue(p.get_nameCheBphosState(),UNPHOS);
	reactantTemplates[1]->addEmptyBindingSite(p.get_nameCheBactiveSite());
	cout<<"here"<<endl;
	
	ReactionClass * r = new ReactionChangeStateOfTwoMolecules("A_PHOS_B", n_reactants, reactantTemplates, 
			p.get_rateCHEA_phos_CHEB(),p.get_nameCheAphosState(),UNPHOS, p.get_nameCheBphosState(), PHOS);
	r->setTraversalLimit(2);
	s->addReaction(r);
}
		...
		NGparam &p)
{
	...
	reactantTemplates[0] = new TemplateMolecule(cheB);
	reactantTemplates[0]->addStateValue(p.get_nameCheBphosState(),PHOS);  //Set constraint that we are at p=1
	reactantTemplates[0]->addEmptyBindingSite(p.get_nameCheBactiveSite());  //Set constraint that we are not interacting with an active site
	
	ReactionClass * r = new ReactionChangeState("B_AUTO_DEPHOS", n_reactants, reactantTemplates, 
			p.get_rateAUTO_dephos_CHEB(),p.get_nameCheBphosState(),UNPHOS);
	r->setTraversalLimit(2);
	s->addReaction(r);
}
