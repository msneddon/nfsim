		...
		NGparam &p)
{
	...
	if(p.get_outputCheAphos() && p.get_useCheA()) { createObservable_CheAphos(cheA,p); } 
	
	if(p.get_outputCheBphos()) {createObservable_CheBphos(cheB,p);}
	if(p.get_outputCheYphos() && !p.get_useMotor()) { createObservable_CheYphos(cheY,p); }
...
//	addCheRB_boundToTetherAndActiveSite(cheR, cheB, receptorDimer);
	...
	if(p.get_outputActivity())
	{
		...
		filenames.push_back((char *)p.get_fileNameGroupActivity());
		values.push_back(P_ON_INDEX);
		
		GroupOutputter * g = new GroupOutputter(s, string(CLUSTER_NAME), string(p.get_fileNameGroupKey()), tm, templateNames, filenames, values);
