...
/* Type of reaction determined by number of rate constants:
    ...
    2.   Michaelis-Menton catalytic reaction:
	   ...
	   total enzyme concentration.  Enzymes need not be listed as
	   products.  Only the first species in both the reactant and product
	   arrays will have its rate affected by the enzyme reaction. [S] is
...
void print_Rxn_array ( FILE *out, Rxn_array *reactions,
	...
	if (rxn->rateLaw_type != ELEMENTARY){
	  fprintf(out, "REACTION_TYPE_GOES_HERE ");
	}
...
static double rxn_rate( Rxn *rxn, double *X, int discrete){
  ...
  case ELEMENTARY:
    ...
    iarr=rxn->r_index;
    // Handle symmetric bimolecular reactions (with discrete molecules)
    if (discrete && (rxn->n_reactants==2) && ((i1=*iarr)==(i2=*(iarr+1)))){
      rate*= X[i1]*(X[i2]-1.0);
    } else {
...
void derivs_network( double t, double *conc, double *derivs){
  ...
  Rxn **rarray, *rxn;
  Elt **rates;
  double x, *X, *dX, rate,rate0, *param;
  int q, n_denom;
  double St, Et, kcat, Km, S, b;
  ...
  for(i=0; i<n_reactions; ++i){
    ...
    case MICHAELIS_MENTON:
      ...
      S= 0.5*(b + sqrt(b*b+4*St*Km));
      rate= rxn->stat_factor*kcat*Et*S/(Km+S);
      /* Set derivatives for first reactant and first product */
      dX[rxn->r_index[0]]-= rate;
      dX[rxn->p_index[0]]+= rate;
      break;
...
FILE *init_print_concentrations_network( char *prefix){
    FILE *out;
    int i, error=0;
    char buf[1000];
    Elt **nconc;
...
int print_concentrations_network( FILE *out, double t){
    ...
    fprintf( out, "%23.16e", t);
    nconc= network.species->elt;
    for(i=0; i<n_species; ++i,++nconc){
      conc= (*nconc)->val;
      fprintf( out, " %23.16e", conc);
    }
    fprintf( out, "\n");
    fflush(out);
...
FILE *init_print_group_concentrations_network( char *prefix){
    FILE *out;
    Group *group;
    int i, error=0;
    char buf[1000];
...
int update_gillespie_direct_network(int n_spec_new, int n_rxns_new){
  ...
  if (n_spec_new){
    ...
    GSP.c_offset= GSP.c - network.species->offset;
    for(i=nc_old; i<GSP.nc; ++i){GSP.c[i]=0.0;}
  }
...
int select_next_rxn(){
    ...
    if (irxn==GSP.na){
      double delta=fabs(a_sum-a_tot);
      if (delta>1e-9){
	printf("Discrepancy between a_sum (%.8g) and a_tot (%.8g indicates numerical problems.\n", a_sum, a_tot);
      }
      --irxn;
    }
    return(irxn);
...
int update_concentrations(int irxn){
	...
	if (newpop<1.0) GSP.c_offset[ri]=0; /* This must be done to
					       avoid negative
					       concentrations! */
    ...
    for(i=0; i<rxn->n_products;++i){
      if (!spec[i]->fixed){
	int pi= rxn->p_index[i];
	double newpop= ++GSP.c_offset[pi];

	if (newpop<=thresh_occ) force_update=1;
      /* Handle species populated for first time */
      /* 	if (GSP.as_reactant_list->l_arr[pii]==0){*/
	if (newpop==1.0 && (GSP.as_reactant_list->l_arr[pi-offset]==0) && first_call){
	  /* 	first_call=0; */
	  ...
	  iarray_add_elt(spec_newpop, 0, pi);
	}
      }
    }
	...
	/* Read group updates */
	read_Groups(  network.spec_groups, stdin, network.species, &line_number,
		      "groups", &n_groups_updated);
...
int gillespie_direct_network( double *t, double delta_t,
    ...
    while (1){
      ...
      if (GSP.a_tot<=0){break;}
      while((rnd=RANDOM(0,1))==0); /* avoid taking log of zero */
